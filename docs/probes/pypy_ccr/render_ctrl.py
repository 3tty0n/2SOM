...
IN = HERE + "/warmup_all_ctrl250.json"
OUT = HERE + "/frontier_ctrl250"
...
def main():
    ...
    for b, v in d.items():
        s0 = v["stock"]
        rows.append((b, v["static"]["curve"][0] / s0["curve"][0], v["ctrl"]["curve"][0] / s0["curve"][0],
                     v["static"]["running"] / s0["running"], v["ctrl"]["running"] / s0["running"]))
    ...
    s.append('<text x="16" y="22" font-size="17" font-weight="bold" fill="#111">CCR on PyPy, full 47-benchmark suite: static CCR vs global tracing-fraction controller (K=250)</text>')
    lx = 16
    for c, nm in ((S, "static CCR"), (A, "controller (TRACEFRAC=250)")):
