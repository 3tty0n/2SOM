2SOM - The Two-Level Simple Object Machine Smalltalk
===========================================

Two-Level Compilation with a Meta-Tracing JIT Compiler
------------

Multi-Tier compilation virtual machine is a VM that has multiple compilers with
different characteristics. Typically, multi-tier compilation VM has a
heavyweight and lightweight compilers. A heavyweight compiler compiles source
code with expensive optimization techniques and generates quality target
code. On the other hand, a lightweight (a.k.a. baseline) compiler generates
target code quickly with inexpensive optimization techniques.

We propose a novel approach to enabling multi-tier compilation in the VMs generated by a meta-
compiler framework. Instead of extending the JIT compiler backend of the framework, our approach
drives an existing (heavyweight) compiler backend in the framework to quickly generate unoptimized
native code by merely embedding directives and compile-time operations into interpreter definitions.

Two-level Simple Object Machine (2SOM) is our implementation of the method for
generating lightweight multi-tier JIT compiler VM in a meta-tracing compiler
frameowork. 2SOM uses threaded code genration as a tier1-compiler and
meta-tracing compiler as a tier2-compiler.

To build 2SOM, please follow the following command.
...
You need to add [`rtime_ext`](https://github.com/prg-titech/rtime_ext) to `PYTHONPATH` build this version of 2SOM.
