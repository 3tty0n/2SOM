...
def _interpret_naive(
    frame, stack, current_bc_idx, entry_bc_idx, method, dummy=False
):
...
def _interpret_nlr_CALL_ASSEMBLER(
    frame, stack, current_bc_idx, entry_bc_idx, invokable, dummy=False
):
...
def get_printable_location_tier1(bytecode_index, entry_bc_idx, method):
...
tier1jitdriver = jit.JitDriver(
    ...
    greens=["current_bc_idx", "entry_bc_idx", "method",],
    reds=["frame", "stack"],
    get_printable_location=get_printable_location_tier1,
    should_unroll_one_iteration=lambda current_bc_idx, entry_bc_idx, method: True,
