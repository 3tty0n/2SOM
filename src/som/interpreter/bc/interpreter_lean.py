"""Shared factory for the tier-4 binary's two lean interpreter siblings.

The warm/stack-inliner interpreter (every send residualized) and the committed
tier-3 interpreter (every send inlined) are byte-for-byte identical except at the
send sites and the merge-point promotion exit. Rather than maintain two ~700-line
clones, both are *derived* from one body by `make_lean_interp`, parameterized by
two plain booleans captured as closure variables:

  * residualize -- True: residualize every send (warm phase, + warm-only mega/
                   operand profiling); False: inline every send (committed tier 3).
  * osr_exit    -- True: read the quasi-immutable adaptive_tier at the merge point
                   and exit to the committed/hybrid interpreter on promotion;
                   False: no read (committed code stays in this graph).

Each call to the factory builds a fresh JitDriver and a distinct flow graph, so
the boolean arms fold away at flow-graph time (RPython turns the captured bool
into a Constant) -- each sibling is exactly as lean as a hand-written clone.

interpreter_lean3.py and interpreter_inliner.py are thin re-export shims over the
two siblings produced here, so existing call sites are unchanged.
"""
...
def make_lean_interp(residualize, osr_exit, gpl_fn, driver_name):
    """Derive one lean interpreter sibling (+ its JitDriver) from the shared body.

    residualize / osr_exit are plain bools, captured as closure variables, so the
    arms below fold at flow-graph time -- the generated graph for each sibling
    contains only its own send policy and merge-point behaviour.
    """
    ...
    @jit.unroll_safe
    def interp(method, frame, max_stack_size, current_bc_idx=0, stack=None, stack_ptr=-1):
        ...
        while True:
            ...
            if osr_exit:
                # Promotion exit: adaptive_tier is quasi-immutable, so the
                # controller's promotion write invalidates the warm trace and the
                # loop continues in the committed interpreter at the same bytecode.
                # Pass len(stack) rather than max_stack_size: keeping the latter
                # live across the merge point would require it as a red, and
                # len(stack) == max_stack_size here.
            ...
            # Committed (osr_exit False) deliberately performs NO adaptive_tier
            # read: inline-mode execution is correct for every tier, and reading
            # it would make each committed trace of a still-undecided method depend
            # on the quasi-immutable -- the 0->2 warm decision and the 2->3
            # promotion would then invalidate and recompile every such trace
            # (triple compilation; Experiment1 0.70 -> 1.02). Routing keeps warm
            # and hybrid activations out of this graph.
            ...
            if bytecode == Bytecodes.dup:
                ...
                if residualize and not we_are_jitted():
                    # mega detection: count receiver classes once the 2-entry IC
                    # overflowed
    ...
    interp.__name__ = "interp_residualize" if residualize else "interp_inline"
    return interp, jitdriver


# Committed tier-3 sibling: inline every send, no merge-point adaptive_tier read.
interpret_lean3, lean3_jitdriver = make_lean_interp(
    residualize=False,
    osr_exit=False,
    gpl_fn=get_printable_location_lean3,
    driver_name="Tier3",
)

# Warm stack-inliner sibling: residualize every send, promotion exit at the merge
# point. Its OSR arm targets interpret_lean3 (above) and interpret_tier3.
interpret_inliner, inliner_jitdriver = make_lean_interp(
    residualize=True,
    osr_exit=True,
    gpl_fn=get_printable_location_inliner,
    driver_name="Inliner",
)
