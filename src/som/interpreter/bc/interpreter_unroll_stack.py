...
def _halt(current_bc_idx, next_bc_idx,  method, frame, stack):
    return stack.top()
...
def _push_1(current_bc_idx, next_bc_idx,  method, frame, stack):
  ...
  # transformation can be done almost automatically!
...
def _interpret_naive(
    ...
    return interpret_unroll_stack(method, frame, 8, dummy)
...
def _interpret_CALL_ASSEMBLER(
    ...
    # if dummy:
    #     return
    return interpret_unroll_stack(method, frame, 8, dummy=dummy)
...
def _q_super_send_4(current_bc_idx, next_bc_idx,  method, frame, stack):
    invokable = method.get_inline_cache_invokable(current_bc_idx)
    arg3 = stack.pop()
    arg2 = stack.pop()
    arg1 = stack.pop()
...
def _is_true_object(current_bc_idx, next_bc_idx,  method, frame, stack):
    val = stack.pop()
    return val is trueObject


def _is_false_object(current_bc_idx, next_bc_idx,  method, frame, stack):
    val = stack.pop()
    return val is falseObject


def _is_greater_two(current_bc_idx, next_bc_idx,  method, frame, stack):
    top = stack.top()
...
@jit.unroll_safe
def interpret_unroll_stack(
    method, frame, max_stack_size, current_bc_idx=0, stack=None,
):
    from som.vm.current import current_universe
    from som.vmobjects.method_bc import BcMethod
    ...
    while True:
        ...
        unroll_stack_jitdriver.jit_merge_point(
        ...
        if bytecode == Bytecodes.dup:
            ...
            unroll_stack_jitdriver.can_enter_jit(
                current_bc_idx=current_bc_idx,
                entry_bc_idx=entry_bc_idx,
                method=method,
                frame=frame,
            ...
            _q_super_send_4(current_bc_idx, next_bc_idx,  method, frame, stack)
...
unroll_stack_jitdriver = jit.JitDriver(
    name="Stack-Unrolling Interpreter",
